...
use gaia_rs::{
    abci_handler::ABCIHandler, client::GaiaQueryCommands, config::AppConfig, genesis::GenesisState, store_keys::{GaiaParamsStoreKey, GaiaStoreKey}, GaiaApplication, GaiaCore
};
use gears::{
    application::{command::app::AppCommands, node::NodeApplication}, baseapp::{run::RunCommand, Genesis}, client::query::{run_query, QueryCommand}, config::{DEFAULT_ADDRESS, DEFAULT_REST_LISTEN_ADDR, DEFAULT_TENDERMINT_RPC_ADDRESS}
};
...
fn account_query() -> anyhow::Result<()> {
    let tendermint = TmpChild::start_tendermint::<_, AppConfig>(TENDERMINT_PATH, &MockGenesis::default())?;
    ...
    let cmd = QueryCommand
    {
        ...
        height: None,
        inner: GaiaQueryCommands::Auth(AuthQueryCli{ command : AuthCommands::Account( query ) }),
    };
