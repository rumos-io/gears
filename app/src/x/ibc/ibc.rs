use database::DB;
use ibc::core::{
    context::Router, ics26_routing::msgs::MsgEnvelope, ExecutionContext, ValidationContext,
};
use proto_types::AccAddress;

use crate::{error::AppError, types::Context};

const KEY_NEXT_CLIENT_SEQUENCE: [u8; 18] = [
...
impl IBCMsg {
    pub fn new(msg: MsgEnvelope) -> Result<IBCMsg, AppError> {
        ...
        Ok(IBCMsg { msg, signers })
    }
}

pub fn run_tx<'a, 'b, T: DB>(ctx: &'a mut Context<'b, T>, msg: IBCMsg) -> Result<(), AppError> {
    let mut ibc_context = IBCExecutionContext { app_ctx: ctx };
    ibc_context
        .execute(msg.msg)
        .map_err(|e| AppError::IBC(e.to_string()))
}
