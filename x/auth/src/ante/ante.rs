...
pub trait BankKeeper<SK: StoreKey>: Clone + Send + Sync + 'static {
    ...
    fn get_denom_metadata<DB: Database, CTX: Context<DB, SK> + ReadContext<DB, SK>>(
        &self,
        ctx: &CTX,
        base: &Denom,
    ) -> Option<Metadata>;
...
pub trait AuthKeeper<SK: StoreKey>: Clone + Send + Sync + 'static {
    ...
    fn has_account<DB: Database, CTX: Context<DB, SK> + ReadContext<DB, SK>>(
        &self,
        ctx: &CTX,
        addr: &AccAddress,
    ) -> bool;
...
impl<BK: BankKeeper<SK>, AK: AuthKeeper<SK>, SK: StoreKey> BaseAnteHandler<BK, AK, SK> {
    ...
    fn validate_memo_ante_handler<
        DB: Database,
        CTX: Context<DB, SK> + ReadContext<DB, SK>,
        M: Message,
    >(
        &self,
        ctx: &CTX,
    ...
    fn sig_verification_handler<
        DB: Database,
        CTX: ContextMut<DB, SK> + ReadContext<DB, SK>,
        M: Message + ValueRenderer,
    >(
        &self,
        ctx: &mut CTX,
