...
impl<MessageDefaultRenderer> MessageValueRenderer<MessageDefaultRenderer> for MsgSend {
    fn format(&self) -> Result<Vec<Screen>, Box<dyn Error>> {
        ...
        for coin in &self.amount {
            screens_vec.append(&mut MessageValueRenderer::<MessageDefaultRenderer>::format(
                coin,
            )?)
        }
...
mod tests {
    ...
    fn screen_result_no_coins() {
        ...
        let msg: MsgSend =
            serde_json::from_str(MESSAGE).expect("Faliled to deserialize test message");
        ...
        let expected_screens: Vec<Screen> =
            serde_json::from_str(SCREENS).expect("Failed to deserialize screens");
