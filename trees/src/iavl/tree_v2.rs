...
impl InnerNode {
    ...
    /// This does three things at once to prevent repeating the same code
...
#[derive(Debug, Clone, PartialEq, Default)]
pub(crate) struct InnerRangeNode {
    pub(crate) left_node: Option<Arc<RangeNode>>, // None means value is the same as what's in the DB
    pub(crate) right_node: Option<Arc<RangeNode>>,
    key: Vec<u8>,
    height: u8,
    size: u32, // number of leaf nodes in this node's subtrees
    pub(crate) left_hash: [u8; 32],
...
impl From<Node> for RangeNode {
    fn from(node: Node) -> Self {
        match node {
            Node::Leaf(node) => RangeNode::Leaf(node),
            Node::Inner(node) => RangeNode::Inner(InnerRangeNode {
                left_node: None,
                right_node: None,
                key: node.key,
                height: node.height,
...
pub(crate) enum RangeNode {
    Leaf(LeafNode),
    Inner(InnerRangeNode),
}

impl RangeNode {
    pub fn get_key(&self) -> &Vec<u8> {
        match self {
            RangeNode::Leaf(leaf) => &leaf.key,
            RangeNode::Inner(inner) => &inner.key,
        }
    }
}
...
{
    range: R,
    //cached_nodes: Vec<BorrowOROwned<'a>>,
    delayed_nodes: Vec<Node>,
    node_db: &'a NodeDB<T>,
}

// enum BorrowOROwned<'a> {
//     Borrowed(&'a Node),
//     Owned(Node),
// }

// impl<'a> BorrowOROwned<'a> {
//     pub fn get_key(&self) -> &Vec<u8> {
//         match self {
//             BorrowOROwned::Borrowed(node) => node.get_key(),
//             BorrowOROwned::Owned(node) => node.get_key(),
//         }
//     }

//     pub fn as_ref(&'a self) -> &'a Node {
//         match self {
//             BorrowOROwned::Borrowed(node) => node,
//             BorrowOROwned::Owned(node) => node,
//         }
//     }

//     pub fn from_owned(node: Node) -> Self {
//         Self::Owned(node)
//     }

//     pub fn from_borrowed(node: &'a Node) -> Self {
//         Self::Borrowed(node)
//     }
// }
                ...
                if before_end {
                    match inner.right_node {
                        ...
                        None => {
                            ...
                            // self.cached_nodes.push(right_node);
                            // self.delayed_nodes.push(
                            //     self.cached_nodes
                            //         .last()
                            //         .expect("this value has just been stored in the cache"),
                            // );
