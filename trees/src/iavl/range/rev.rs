...
impl<DB: Database, R: RangeBounds<RB>, RB: AsRef<[u8]> + Debug> RevRange<'_, DB, RB, R> {
    fn traverse(&mut self) -> Option<(Vec<u8>, Vec<u8>)> {
        ...
        let node = self.delayed_nodes.pop_front()?;

        let after_start = match self.range.start_bound() {
            Bound::Included(l) => {
                // dbg!(l.as_ref(), node.get_key(), node.get_key() >= l.as_ref());
                node.get_key() > l.as_ref()
            }
        ...
        let before_end = match self.range.end_bound() {
            ...
            Bound::Excluded(u) => {
                // dbg!(u.as_ref(), node.get_key(), node.get_key() < u.as_ref());
                node.get_key() < u.as_ref()
            }
            Bound::Unbounded => true,
        };
            ...
            Node::Inner(inner) => {
                ...
                if before_end {
                    match inner.left_node {
                        Some(left_node) => self.delayed_nodes.push_front(left_node),
                        None => {
                            let left_node = self
                                .node_db
                            ...
                            self.delayed_nodes.push_back(left_node);
                        }
                    }
                }
                ...
                if after_start {
                    match inner.right_node {
                        Some(right_node) => self.delayed_nodes.push_front(right_node),
                        None => {
                            let right_node = self
                                .node_db
                            ...
                            self.delayed_nodes.push_back(right_node);
                        }
                    }
                }
            }
